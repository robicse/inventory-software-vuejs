...
class TaxController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($is_default == 1) {
            ...
            if (Tax::store($taxItem)) {
                $response = [
                    'message' => Lang::get('lang.vat_item') . ' ' . Lang::get('lang.successfully_added')
                ];
    ...
    public function update(Request $request, $id)
    {
        ...
        if (Tax::updateData($id, $taxItem)) {
            ...
            $response = [
                'message' => Lang::get('lang.vat_item') . ' ' . Lang::get('lang.successfully_updated')
    ...
    public function deleteTax($id)
    {
        ...
        if ($used == 0) {
            Tax::deleteData($id);
            $response = [
                'message' => Lang::get('lang.vat_item') . ' ' . Lang::get('lang.successfully_deleted')
            ];

            return response()->json($response, 200);
        } else {
            $response = [
                'message' => Lang::get('lang.vat_item') . ' ' . Lang::get('lang.in_use') . ', ' . Lang::get('lang.you_can_not_delete_the') . ' ' . strtolower(Lang::get('lang.vat_item'))
            ];
