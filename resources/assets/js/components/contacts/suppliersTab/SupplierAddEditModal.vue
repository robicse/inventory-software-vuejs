        ...
        <div class="modal-layout-content">
            ...
            <form class="form-row" v-else>
                <div class="form-group col-md-6">
                    ...
                    <input v-validate="'required'" name="firstname" data-vv-as="first name" id="first_name" type="text"
                    ...
                    <div v-if="submitted && errors.has('firstname')" class="heightError">
                        <small class="text-danger" v-show="errors.has('firstname')">{{ errors.first('firstname') }}
                        </small>
                    </div>
                </div>
                <div class="form-group col-md-6">
                    ...
                    <div v-if="submitted && errors.has('lastname')" class="heightError">
                        <small class="text-danger" v-show="errors.has('lastname')">{{ errors.first('lastname') }}
                        </small>
                    </div>
                </div>
                <div class="form-group margin-top col-md-12">
                    <label for="email">{{ trans('lang.customer_email') }}</label>
                    <input v-validate="'required|email'" name="email" id="email" type="text" class="form-control"
                           v-model="email" :class="{ 'is-invalid': submitted && errors.has('email') }">
                ...
                <div class="form-group margin-top col-md-6">
                    ...
                    <input name="phonenumber" id="phoneNumber" type="text" class="form-control" v-model="phoneNumber"
                           v-validate="'phoneNumber'">
                    <div class="heightError">
                        <small class="text-danger" v-show="errors.has('phonenumber')">{{ errors.first('phonenumber')
                            }}
                        </small>
                    </div>
                </div>
            ...
            save() {
                ...
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        ...
                        if (this.id) {
                            this.postDataMethod('/supplier/' + this.id, this.inputFields);
                        }
                        else {
                            this.postDataMethod('/supplier/store', this.inputFields);
                        }
                    }
                });
            },
            getSupplierData(route) {
                ...
                this.axiosGet(route,
                    function (response) {
                        ...
                        instance.phoneNumber = response.data.supplierData.phone_number;
