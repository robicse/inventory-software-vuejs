        ...
        <div v-show="hidePreLoader">
            ...
            <div class="form-row">
                <div v-for="(filter, index) in options.filters"
                     :class="{'col-md-6': (filtersData.length === 1 && options.search), 'col-12 col-md-4 col-lg-4': (filtersData.length % 3 === 0 && !options.search || filtersData.length  === 5 )||(filtersData.length % 2 ===0 && options.search), 'col-12 col-md-6 col-lg-3': (filtersData.length % 2 ===0 && !options.search)||(filtersData.length % 3 === 0 && options.search) }">
                    <!-- Date Filter -->
                    ...
                    <div class="dateRangeFilter" v-if="filter.type === 'date_range'" :key="filter.key">
                        ...
                        <date-filter :id="filter.key" :label="filter.title" :format="dateFormat" :dateClear="dateClear"
                                     @setDatefilterValue="getDatefilterValue"
                                     @resetdateClears="resetDateClears"></date-filter>
                    </div>
                    <!-- Other Dropdown Filter -->
                    <div class="input-field" v-show="filter.type ==='dropdown'">
                        <select :id="'filter_'+index" class="form-control">
                ...
                instance.axiosPost(url, {
                    ...
                    function (response) {
                        ...
                        if (instance.reqType === '') {
                            ...
                            //console.log("totalResultCount="+instance.totalResultCount);
                            //console.log("datasets="+instance.datasets.length);
