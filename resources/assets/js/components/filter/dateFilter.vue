<template>
    <div class="calendar-root">
        <div class="form-control" @click="toggleCalendar()"> {{getDateString(dateRange.start)}} - {{getDateString(dateRange.end)}}</div>
        <div class="calendar" v-click-outside="nameOfCustomEventToCall" :class="{'calendar-mobile ': isCompact, 'calendar-right-to-left': isRighttoLeft}" v-if="isOpen">
            ...
            <div class="calendar-range" :class="{'calendar-range-mobile ': isCompact}" v-if="!showMonth || !isCompact">
                <ul class="calendar_preset">
                    ...
                    <li><button class="btn calendar-btn-apply"  @click="setDateValue()" :disabled="dateRange.end == undefined">{{captions.ok_button}}</button></li>
                </ul>
            </div>
